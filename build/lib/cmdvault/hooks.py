...
# New branded markers
BASH_MARK_START = "# >>> repty bash hook >>>"
BASH_MARK_END = "# <<< repty bash hook <<<"
ZSH_MARK_START = "# >>> repty zsh hook >>>"
ZSH_MARK_END = "# <<< repty zsh hook <<<"
PS_MARK_START = "# >>> repty powershell hook >>>"
PS_MARK_END = "# <<< repty powershell hook <<<"

# Legacy markers retained for cleanup/compatibility
LEGACY_BASH_MARK_START = "# >>> cmdvault bash hook >>>"
LEGACY_BASH_MARK_END = "# <<< cmdvault bash hook <<<"
LEGACY_ZSH_MARK_START = "# >>> cmdvault zsh hook >>>"
LEGACY_ZSH_MARK_END = "# <<< cmdvault zsh hook <<<"
LEGACY_PS_MARK_START = "# >>> cmdvault powershell hook >>>"
LEGACY_PS_MARK_END = "# <<< cmdvault powershell hook <<<"


def bash_snippet() -> str:
    body = (
        r"""
# Repty: capture commands and log after execution
...
def zsh_snippet() -> str:
    body = (
        r"""
# Repty: capture commands via zsh hooks
# Only run in interactive shells
...
def powershell_snippet() -> str:
    body = (
        r"""
if (-not (Get-Command repty -ErrorAction SilentlyContinue)) { return }
...
function global:__CmdVault_LogLast {
  try {
    ...
    if ($null -ne $hist -and $hist.Id -ne $global:CmdVaultLastHistoryId) {
      ...
      if ($cmd -and -not $cmd.StartsWith('repty ')) {
        repty log --command $cmd --cwd $cwd --exit $exit | Out-Null
      }
...
def install(bashrc: Optional[Path] = None, zshrc: Optional[Path] = None) -> str:
    ...
    msgs = []
    # Avoid duplicate installs if legacy blocks exist
    bash_content = bashrc.read_text(encoding="utf-8") if bashrc.exists() else ""
    zsh_content = zshrc.read_text(encoding="utf-8") if zshrc.exists() else ""
    if (BASH_MARK_START in bash_content and BASH_MARK_END in bash_content) or (
        LEGACY_BASH_MARK_START in bash_content and LEGACY_BASH_MARK_END in bash_content
    ):
        ...
        if _add_snippet(bashrc, bash_snippet(), BASH_MARK_START, BASH_MARK_END):
            ...
            msgs.append(f"Bash hook already present in {bashrc}")
    if (ZSH_MARK_START in zsh_content and ZSH_MARK_END in zsh_content) or (
        LEGACY_ZSH_MARK_START in zsh_content and LEGACY_ZSH_MARK_END in zsh_content
    ):
...
def uninstall(bashrc: Optional[Path] = None, zshrc: Optional[Path] = None) -> str:
    ...
    zshrc = zshrc or home / ".zshrc"
    msgs = []
    # Remove both new and legacy blocks
    if _remove_snippet(bashrc, BASH_MARK_START, BASH_MARK_END) or _remove_snippet(bashrc, LEGACY_BASH_MARK_START, LEGACY_BASH_MARK_END):
        msgs.append(f"Removed Bash hook from {bashrc}")
    if _remove_snippet(zshrc, ZSH_MARK_START, ZSH_MARK_END) or _remove_snippet(zshrc, LEGACY_ZSH_MARK_START, LEGACY_ZSH_MARK_END):
        msgs.append(f"Removed Zsh hook from {zshrc}")
    for p in _ps_profile_paths():
        try:
            if _remove_snippet(p, PS_MARK_START, PS_MARK_END) or _remove_snippet(p, LEGACY_PS_MARK_START, LEGACY_PS_MARK_END):
