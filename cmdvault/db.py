import os
if os.environ.get("CMDVAULT_SQLITE_BACKEND") == "pysqlite3":
    try:
...
def _ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    try:
        ...
        if "fts5" in str(e).lower():
            raise RuntimeError(
                "Your Python's SQLite does not support FTS5. Install a build with FTS5 (e.g., Python 3.11+), or install 'pysqlite3-binary' and run CMDVAULT_SQLITE_BACKEND=pysqlite3."
            ) from e
        else:
            raise
        ...
        cur.execute(
            ...
            WITH matched AS (
              ...
              WHERE commands_fts MATCH ?
                AND c.command NOT LIKE 'cmdvault %'
...
def last_user_command(conn: sqlite3.Connection) -> Optional[sqlite3.Row]:
    """Return the most recent non-internal command (skips cmdvault/source/dot)."""
    cur = conn.cursor()
    cur.execute(
        (
            "SELECT id, command, cwd, timestamp, exit_code, tags FROM commands "
            "WHERE command NOT LIKE 'cmdvault %' AND command NOT LIKE 'source %' AND command NOT LIKE '. %' "
...
def search_by_tags(conn: sqlite3.Connection, tokens: List[str], limit: int = 50) -> List[sqlite3.Row]:
    ...
    sql = (
        "SELECT id, command, cwd, timestamp, exit_code, tags FROM commands "
        f"WHERE ({cond}) AND command NOT LIKE 'cmdvault %' AND command NOT LIKE 'source %' AND command NOT LIKE '. %' "
